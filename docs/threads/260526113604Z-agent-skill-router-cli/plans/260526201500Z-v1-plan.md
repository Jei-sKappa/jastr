...
```bash
bun run test test/scaffold.test.ts
```
...
```bash
bun run test test/errors.test.ts
```
...
```bash
bun run test test/cli-args.test.ts
```
...
```bash
bun run test test/project-skill.test.ts
```
...
```bash
bun run test test/frontmatter-schema.test.ts
```
...
```bash
bun run test test/flags.test.ts
```
...
```bash
bun run test test/conditions.test.ts
```
...
```bash
bun run test test/directives.test.ts
```
...
function parseDirectiveOpening(line: string):
  ...
  if (rest !== "" && !(rest.startsWith("{") && rest.endsWith("}"))) {
    throw new SkillrouterError("invalid_frontmatter", `Invalid directive attributes ${rest}.`);
  }
...
```bash
bun run test test/includes.test.ts
```
...
```bash
bun run test test/interpolation.test.ts
```
...
```bash
bun run test test/render.test.ts
```
...
```bash
bun run test test/cli-integration.test.ts
```
...
Update the existing `test/includes.test.ts` include import to add `detectIncludeCycle`:
...
```bash
bun run test test/conditions.test.ts test/includes.test.ts test/flags.test.ts test/render.test.ts test/cli-integration.test.ts
```
...
If the include cycle test fails because `detectIncludeCycle` is not imported, update the existing include import exactly as shown in Step 1.
