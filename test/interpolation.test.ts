...
} from "../src/compiler/interpolation";
...
describe("interpolation", () => {
  it("interpolates declared direct input placeholders", () => {
    expect(
      interpolateText(
        "Analyze {{target-file}} as {{language}}.",
        schema,
        values,
      ),
    ).toBe("Analyze src/index.ts as typescript.");
  });

  it("validates interpolation references statically", () => {
    expect(() =>
      validateInterpolationReferences("{{language}}", schema),
    ).not.toThrow();
    expect(() =>
      validateInterpolationReferences("{{missing}}", schema),
    ).toThrow("Interpolation references undeclared input missing.");
    expect(() =>
      validateInterpolationReferences("{{language | upper}}", schema),
    ).toThrow("Invalid interpolation reference language | upper.");
  });

  it("fails when an optional input is interpolated but absent", () => {
    expect(() =>
      interpolateText("Analyze {{target-file}}.", schema, {
        language: "typescript",
      }),
    ).toThrow(
